                ...
                <div class="col-md-6 mb-2 mt-2">
                  <form action=""  class="form-inline ml-2">
                      <div class="field">
                          <div class="control is-expanded has-icons-right">
                              ...
                              value="{{request()->get('cari_pendaftar_akhwat')}}"


                              placeholder="Cari Pendaftar Kajian"

                               name="cari_pendaftar_akhwat"
                  ...
                  <table class="table table-borderless table-striped table-earning ">
                      <thead>
                        ...
                        @if($count_pendaftar_akhwat > 0)
                        @foreach($pendaftar_akhwat as $data => $hasil)
                      <tbody>
                          <tr>
                              <td>{{$data + $pendaftar_akhwat->firstitem()}}</td>
                              <td>{{$hasil->nama}}</td>
                              <td>{{$hasil->email}}</td>
                      ...
                      @else
                      <tbody>
                          <td colspan="11" class="text-center">Tidak Ada Pendaftar Akhwat</td>
                      </tbody>
                      @endif
                  </table>
                  <div class="d-flex justify-content-center mt-2 mb-2">
                    {{ $pendaftar_akhwat->appends(request()->query())->links() }}
                  </div>
          </div>
